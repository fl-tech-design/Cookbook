...
def cr_but_back(self, master):
    tk.Button(self, text=r_data(language, 'chancel'), font=fo_sma, bg=bg_app, fg=fg_app,
...
def cr_fr_ma(win, master, ts1, ts2, r, c):
    ...
    cr_but_page(fr_main, master, r_data(language, 'new_rec'), fo_mid, bg_app, fg_app, ts1, 1, 0, 20, 0, 0, 1, 1)
    cr_but_page(fr_main, master, r_data(language, 'edit_rec'), fo_mid, bg_app, fg_app, ts2, 1, 1, 20, 2, 0, 1, 1)
...
def cr_view_ingr(win, recipe, stat=0):
    ...
    cr_lab(fr_prep, r_data(language, 'ingredients'), fo_big, bg_app, fg_app, 0, 0, 0, 0, 1, 1, 'nw')
...
def cr_view_prep(self, recipe, stat=0):
    ...
    cr_lab(self.fr_prep, r_data(language, 'preparation'), fo_big, bg_app, fg_app, 0, 0, 0, 0, 1, 1, 'nw')
...
def cr_win_tit(win, text, spoon, logo):
    cr_lab(win, r_data(language, text), fo_tit, bg_app, fg_app, 1, 1, 0, 0, 3, 16, 'w')
...
class CB(tk.Tk):
    ...
    def cr_menubar(self, fo, bg, fg, bga, fga):
        ...
        self.filemenu.add_command(label=r_data(language, 'new_rec'),
                                  command=lambda: self.master.switch_frame(PageNewRec))
        self.filemenu.add_command(label=r_data(language, 'edit_rec'),
                                  command=lambda: self.master.switch_frame(PageEditRec))
        self.filemenu.add_command(label=r_data(language, 'quit'), command=self.quit)

        self.menubar.add_cascade(label=r_data(language, 'file'), menu=self.filemenu)

        self.helpmenu = tk.Menu(self.menubar, tearoff=0, font=fo, bg=bg, fg=fg, activebackground=bga,
                                activeforeground=fga, relief='flat')
        self.helpmenu.add_command(label=r_data(language, 'about'))
        self.helpmenu.add_command(label=r_data(language, 'help'))
        self.menubar.add_cascade(label=r_data(language, 'help'), menu=self.helpmenu)
...
class PageStart(tk.Frame):
    ...
    def __init__(self, master):
        ...
        cr_win_tit(self, 'la_titel', self.pi_spo, self.pi_log)
        cr_lines_ps(self)
        # create a field for act_pic frame and Label
        cr_lab(self, r_data(language, 'la_titel'), fo_big, bg_app, fg_app, 10, 1, 10, 0, 2, 9, 'w')
    ...
    def fr_choose_rec(self, r, c):
        self.fr_choo = tk.Frame(self, bg=bg_app, relief='flat')
        self.fr_choo.grid(row=r, column=c, rowspan=10, columnspan=14)
        cr_lab(self.fr_choo, r_data(language, 'la_choos'), fo_big, bg_app, fg_app, 0, 0, 0, 0, 1, 1, 'w')
    ...
    def set_act_rec_pic(self):
        self.pi_rec_a['file'] = pa_rec + '/' + self.act_rec_name_var.get() + '.png'
...
if __name__ == "__main__":
    ...
    fo_tit = r_di_val(pa_fon, 'Cal24')
    fo_big = r_di_val(pa_fon, 'Cal14')
    fo_mid = r_di_val(pa_fon, 'Cal10')
    fo_sma = r_di_val(pa_fon, 'Cal8')
